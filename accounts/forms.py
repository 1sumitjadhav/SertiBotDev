...
class universitySignUpForm(UserCreationForm):
    ...
    def data_save(self):
        ...
        user.save()
        University = university.objects.create(user=user)
        University.phone_number = self.cleaned_data.get('phone_number')
        University.department = self.cleaned_data.get('department')
        University.save()
        return user